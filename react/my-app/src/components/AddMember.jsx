...
const AddMember = () => {
  ...
  const fetchMembers = async () => {
    try {
      const res = await axios.post("http://localhost:2000/api/getallmember");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editMode
        ? "http://localhost:2000/api/updatemember"
        : "http://localhost:2000/api/createmember";
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure?")) {
      try {
        await axios.post("http://localhost:2000/api/deletememberbyid", { id });
                ...
                <tbody>
                  {memberList.map((member) => (
                    <tr key={member._id}>
                      ...
                      <td>
                        {member.image && (
                          <img src={`http://localhost:2000/uploads/${member.image}`} alt="img" width="50" />
                        )}
                      </td>
                      ...
                      <td>
                        {member.addressProof && (
                          <a href={`http://localhost:2000/uploads/${member.addressProof}`} target="_blank" rel="noreferrer">
                            View
                          </a>
                        )}
                      </td>
                      <td>
                        {member.idProof && (
                          <a href={`http://localhost:2000/uploads/${member.idProof}`} target="_blank" rel="noreferrer">
                            View
                          </a>
                        )}
                      </td>
