...
const FundRaise = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editMode) {
       ...
       const updated =  await axios.post("http://localhost:2000/api/updatefundbyid",{id:editingId,...formData});
       toast.success("fund updated successfully")
      } else {
        const add =   await axios.post('http://localhost:2000/api/addfund', formData);
        toast.success("fund is added successfully")
      }
      setFormData({ title: '', description: '', startDate: '', endDate: '', amount: '', status: '' });
      setEditMode(false);
      setEditingId(null);
      fetchFunds();
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    const deleteUser =  await axios.post("http://localhost:2000/api/deletefund",{id});
    fetchFunds();
